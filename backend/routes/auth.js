  ...
  try {
    ...
    if (type === "mobile") {
      if (!mobile?.trim() || !code) {
        return res
          .status(400)
          .json({ message: "Mobile number and code are required" });
      }
...
// Forgot Password - Check if email exists and return user info
router.post("/forgot-password/check", async (req, res) => {
  try {
    ...
    res.status(500).json({ message: err.message || "Failed to check email" });
  }
});
